...
const inputFilePath = "input720p.mp4";
...
(async () => {
    try {
        ...
        const inputResolution = { width: convertedResolution.width, height: convertedResolution.height };
...
async function generateMasterPlaylist(resolutions) {
    ...
    const masterContent = `#EXTM3U\n${playlists
        .map(
            (playlist) =>
                `#EXT-X-STREAM-INF:BANDWIDTH=${getBandwidth(playlist.resolution)},RESOLUTION=${playlist.resolution}\n${playlist.path}`
        )
        .join("\n")}`;
...
function getBandwidth(resolution) {
    ...
    const bitrate = resolutions.find((res) => res.width === parseInt(width) && res.height === parseInt(height)).bitrate;

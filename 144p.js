...
getVideoInfo(inputFilePath)
    .then(({ frameRate, totalFrames }) => {
        ffmpeg()
            .input(inputFilePath)
            .videoFilter("scale=256x144:flags=lanczos")
            .videoCodec("libx264")
            .outputOptions("-profile:v main")
            .outputOptions("-preset:v medium")
            .videoBitrate("120k")
            .outputOptions("-maxrate 120k")
            .outputOptions("-bufsize 240k")
            .outputOptions("-g 60")
            .audioCodec("aac")
            .audioBitrate("64k")
            .audioChannels(2)
            ...
            .outputOptions(`-hls_segment_filename ${outputDirectory}/144p_%03d.ts`)
            .output(`${outputDirectory}/output_144p.m3u8`)
